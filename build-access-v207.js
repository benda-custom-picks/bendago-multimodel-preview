/* BCP V206 — preserves the chosen model look through Build Access checkout and return. */
...
(function(){
  ...
  function checkoutReturnPath(){
    var look = selectedLookCard();
    if(scope === 'model' && look && look.id){
      /* Keep the return on the exact showroom look. The Worker persists this same-origin relative path. */
      return (location.pathname || '/') + '#' + encodeURIComponent(look.id);
    }
    return cleanPath();
  }
  function checkout(button){
    ...
    api('/build-access/create-checkout-session',{method:'POST',body:JSON.stringify({source_page:checkoutReturnPath(),source_build_key:sourceBuildKey(),source_build_name:sourceBuildName()})})
      .then(function(data){
    ...
    document.addEventListener('click',function(event){
      ...
      if(lookCta){
        ...
        if(root.classList.contains('bcp-access-granted')){
          /* The private look is mounted asynchronously. Do not let the browser jump to an
             anchor that does not exist yet; hand the selected look to the catalog runtime. */
          event.preventDefault();
          ...
          try{
            window.dispatchEvent(new CustomEvent('bcp:private-look-request',{detail:{look_id:grantedContext}}));
          }catch(error){}
          return;
        }
